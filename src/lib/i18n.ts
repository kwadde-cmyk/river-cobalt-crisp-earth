...
const de: Record<string, string> = {
  ...
  "stages.must": "muss",
  "stages.keyHint": "Antippen: in der Stufe / raus. Pfeile ändern die Reihenfolge in multi() — das ändert die Checksumme. 2-von-2 brauchen kein »muss«.",
  ...
  "keys.dialogBlurb": "Master zuerst, dann Childkeys. Details ist die Übersicht — bearbeiten in den anderen Tabs.",
  ...
  "keys.detailsTogether": "Nur Übersicht. Name und Fingerprint stehen oben. Bearbeiten unter Master bzw. Childkeys.",
  ...
  "read.ordersHint": "Bei multi() ändert die Pubkey-Reihenfolge Script, Adresse und Checksumme. Antippen übernimmt die Variante.",
  "read.orderSearch": "Checksum suchen…",
  ...
  "read.sortedNote": "sortedmulti sortiert die Pubkeys fest: eine Checksumme, einfachere Wiederherstellung. Nachteil: passt nicht zu einer alten Wallet, die mit unsortiertem multi() in bestimmter Reihenfolge gebaut wurde — Adressen wären andere. On-Chain-Kosten sind gleich. Ledger erwartet sortedmulti.",
  ...
  "node.startos.help": "StartOS: 1. Bitcoin Core → Interfaces → RPC → .local-https kopieren. 2. Adresse einmal im Tab öffnen, Zertifikat bestätigen. 3. Unter Aktionen RPC-Nutzer anlegen, dann Verbinden.",
...
const en: Record<string, string> = {
  ...
  "stages.must": "must",
  "stages.keyHint": "Tap to include or drop a key. Arrows change multi() order — that changes the checksum. 2-of-2 needs no “must”.",
  ...
  "read.ordersHint": "With multi(), pubkey order changes script, address, and checksum. Tap a row to apply it.",
  "read.orderSearch": "Search checksum…",
  "read.orderNone": "No listed order matches this checksum.",
  "read.useOrder": "Use this",
  "read.orderCurrent": "current",
  "read.sortedNote": "sortedmulti sorts pubkeys: one checksum, easier recovery. Downside: it will not match an old wallet built with unsorted multi() in a specific order — addresses would differ. On-chain cost is the same. Ledger expects sortedmulti.",
  ...
  "node.startos.help": "StartOS: 1. Bitcoin Core → Interfaces → RPC → copy the .local https address. 2. Open it once in a tab and trust the certificate. 3. Generate an RPC user under Actions, then Connect.",
